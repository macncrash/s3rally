...
#ifndef __EMSCRIPTEN__  // the browser build has no score client yet
...
void ScoreClient::send(const std::string& kind, const std::string& method, const std::string& path, const std::string& body, bool auth) {
    ...
    (void)method, (void)path, (void)body, (void)kind;
    call->done = true;
#else
    const std::string url = url_ + path;
    const std::string token = auth ? token_ : std::string();
    call->kind = kind;
    call->worker = std::thread([call, url, method, body, token] {
        // Headers and body go through private temp files, never the command line:
        // the token would show in the process list, and nothing here reaches a shell unquoted.
...
void ScoreClient::finished(Call& c) {
    ...
    if (c.code == 401 && c.kind != "register") {  // the server doesn't know this token any more
        token_.clear();
        save();
    }
}
...
void ScoreClient::submit(int stage, double score, const std::string& replay, const std::string& build) {
    ...
    std::string body = ticket_.empty() || stage != ticketStage_
                           ? "{\"game\":" + jsonStr(game_) + ",\"stage\":" + std::to_string(stage)
...
void ScoreClient::rate(int thumb) {
    if (!slugOk(game_)) return;
    send("rate", "PUT", "/v1/ratings/" + game_, std::string("{\"thumb\":") + (thumb > 0 ? "1" : "-1") + "}", true);
}

void ScoreClient::forget() {
    send("forget", "DELETE", "/v1/players/me", "", true);
}
